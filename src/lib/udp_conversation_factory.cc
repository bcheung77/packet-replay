...
namespace packet_replay {
    ...
    std::pair<std::string, TargetTestServer*> UdpConversationFactory::createTargetTestServer(const char* spec) {
        std::string src_addr;
    ...
    std::vector<std::string> UdpConversationFactory::getTargetTestServerKeys(const TransportPacket& packet) {
    ...
    std::string UdpConversationFactory::getKey(const TransportPacket& packet) {
        Layer3* layer3 = dynamic_cast<Layer3 *>(packet.getLayer(NETWORK));
        TcpLayer* tcpLayer = dynamic_cast<TcpLayer *>(packet.getLayer(TRANSPORT));
    ...
    TcpConversation* UdpConversationFactory::createConversation(const TransportPacket& packet, const TargetTestServer* test_server) {
